...
- Rig Graph：7 个模块、97 个声明对象、42 条物理行为，共 139 项可读回构建结果；
...
- 5 套由 Maya 2025 确定性生成的演示场景，以及宽版、窄版、成功、阻断和 Undo 真实截图；
- 2.2 发布节点的 68 项离线测试、相关 mayapy 回归、隐藏 Maya 生命周期和干净安装验证均已通过；
  Bendy 首个算法切片另有 5 项针对性测试通过。
...
下一个 Hero 是 Bendy「形变弧场」。它不是传统的参数表单，而是一张直接表达肢体轮廓的弧线工作区：绑定师拖动弧心和切线意图，先看到零写入形变带，再明确应用到真实 Maya DG。它必须与 Twist 同时存在，弯曲负责轮廓，扭转负责截面朝向，两者互不污染。

前两个切片已落地：三次曲线弧长均匀采样、Parallel Transport Frame、低曲率参考轴回退和可调体积保持均已通过针对性测试；生产 `BendyArcField` 已接入 Rig Graph，可拖两个切线把手或肌腹把手塑造 C/S 型轮廓，并在弧线上显示等距关节、弧长、伸长比与截面比例。Maya 2025 mayapy 已复核 760px、330px 与 Rig Graph 集成尺寸，当前仍保持 Maya 零写入。
