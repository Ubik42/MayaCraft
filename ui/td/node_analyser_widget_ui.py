...
from core.logic.td import node_analyser_widget_logic as logic
...
class ColorManagerDialog(QtWidgets.QDialog):
    ...
    def _populate_ui(self):
        ...
        for node_type, color_hex in self.type_color_map.items():
            ...
            btn.setStyleSheet(f"background-color: {color_hex}; border: none; height: 20px;")
    ...
    def _pick_color(self, node_type, btn):
        ...
        if color.isValid():
            ...
            btn.setStyleSheet(f"background-color: {hex_color}; border: none; height: 20px;")
...
class NodeAnalyserWidget(CollapsibleWidget):
    ...
    def _create_toolbar(self, layout):
        ...
        self.analyse_btn.setIcon(self.style().standardIcon(QtWidgets.QStyle.SP_BrowserReload))

        self.color_btn = QtWidgets.QPushButton(" 颜色配置")
        self.color_btn.setIcon(self.style().standardIcon(QtWidgets.QStyle.SP_ComputerIcon))
    ...
    def _create_table(self, layout):
        ...
        self.node_table.setHorizontalHeaderLabels(["Node Name", "Node Type", "Belong to Sets"])
    ...
    def _populate_table(self, data_list):
        ...
        for row, info in enumerate(data_list):
            ...
            sets_str = ",".join(node_sets) if isinstance(node_sets, list) else str(node_sets)
    ...
    def _filter_table(self):
        ...
        for info in self.all_node_data:
            # Type 检查
            type_match = (target_type == "All Types") or (info.get('type') == target_type)
